...
- **Phase B (required by Tasks 1–2):** `groups.icon` (TEXT, nullable) and
  ...
  ALTER pattern in `server/src/db/schema.ts`. Gate: `grep -n "sort_order" $HOME/github/uber-wled/server/src/db/schema.ts`
...
**Files:**
- Modify: `$HOME/github/uber-wled/server/src/groups/repository.ts` (whole file, currently 57 lines)
- Modify: `$HOME/github/uber-wled/server/src/groups/routes.ts` (POST `/` at lines 11–14)
- Test: `$HOME/github/uber-wled/server/test/groups/routes.test.ts` (append to existing describe)
...
- [ ] Gate: `grep -n "sort_order" $HOME/github/uber-wled/server/src/db/schema.ts` — must show the Phase B idempotent adds for `groups.icon` and `groups.sort_order`. If empty, STOP (Phase B incomplete).
...
- [ ] Run: `cd $HOME/github/uber-wled/server && npm test -- test/groups/routes.test.ts` — expect the 3 new tests to fail (`expected undefined to be null` on `icon`, and `['Alpha','Zeta']` instead of `['Zeta','Alpha']`).
...
- [ ] Run: `cd $HOME/github/uber-wled/server && npm test -- test/groups/routes.test.ts` — all tests pass (3 new + 3 pre-existing).
- [ ] Run full server suite: `cd $HOME/github/uber-wled/server && npm test` — green (schedule/calendar tests construct groups via `repo.add` whose new `icon` field is optional, so no fallout expected; fix any typed fixture that constructs `Group` literals by adding `icon: null, sortOrder: 0`).
- [ ] Commit: `git -C $HOME/github/uber-wled add server/src/groups server/test/groups && git -C $HOME/github/uber-wled commit -m "server: groups carry icon and sortOrder (Home v2 rooms)"`

## Task 2: Server — `POST /api/groups/reorder`

**Files:**
- Modify: `$HOME/github/uber-wled/server/src/groups/routes.ts` (add route above `router.patch('/:id', …)`)
- Test: `$HOME/github/uber-wled/server/test/groups/routes.test.ts` (append)
...
- [ ] Run: `cd $HOME/github/uber-wled/server && npm test -- test/groups/routes.test.ts` — the 2 new tests fail (404 from Express: no `/reorder` route).
...
- [ ] Run: `cd $HOME/github/uber-wled/server && npm test -- test/groups/routes.test.ts` — all pass.
- [ ] Run full suite: `cd $HOME/github/uber-wled/server && npm test` — green.
- [ ] Commit: `git -C $HOME/github/uber-wled add server/src/groups/routes.ts server/test/groups/routes.test.ts && git -C $HOME/github/uber-wled commit -m "server: POST /api/groups/reorder persists room tile order"`

## Task 3: Client lib — `aggregateTileStatusLive` (live-stream tile aggregation)

**Files:**
- Modify: `$HOME/github/uber-wled/client/src/lib/tileStatus.ts` (append new types + function; existing exports untouched until Task 12)
- Test (Create): `$HOME/github/uber-wled/client/src/test/lib/tileStatusLive.test.ts`
...
describe('aggregateTileStatusLive', () => {
  ...
  it('handles the real-device shape captured from 203.0.113.86', () => {
...
- [ ] Run: `cd $HOME/github/uber-wled/client && npm test -- src/test/lib/tileStatusLive.test.ts` — fails: `aggregateTileStatusLive` is not exported.
...
- [ ] Run: `cd $HOME/github/uber-wled/client && npm test -- src/test/lib/tileStatusLive.test.ts` — all 7 pass.
- [ ] Run full client suite: `cd $HOME/github/uber-wled/client && npm test` — green (old `aggregateTileStatus` untouched).
- [ ] Commit: `git -C $HOME/github/uber-wled add client/src/lib/tileStatus.ts client/src/test/lib/tileStatusLive.test.ts && git -C $HOME/github/uber-wled commit -m "client: live-stream tile status aggregation for Home v2"`

## Task 4: Client lib — `dominantColor` glow helper + `throttle`

**Files:**
- Create: `$HOME/github/uber-wled/client/src/lib/dominantColor.ts`
- Create: `$HOME/github/uber-wled/client/src/lib/throttle.ts`
- Test (Create): `$HOME/github/uber-wled/client/src/test/lib/dominantColor.test.ts`
- Test (Create): `$HOME/github/uber-wled/client/src/test/lib/throttle.test.ts`
...
describe('dominantColor', () => {
  ...
  it('never dims below the visibility floor (real device fixture, master bri 9)', () => {
    // captured 2026-07-04 from GET http://203.0.113.86/json/state:
...
- [ ] Run: `cd $HOME/github/uber-wled/client && npm test -- src/test/lib/dominantColor.test.ts src/test/lib/throttle.test.ts` — both fail: "Failed to resolve import".
...
- [ ] Run: `cd $HOME/github/uber-wled/client && npm test -- src/test/lib/dominantColor.test.ts src/test/lib/throttle.test.ts` — all pass.
- [ ] Run full client suite: `cd $HOME/github/uber-wled/client && npm test` — green.
- [ ] Commit: `git -C $HOME/github/uber-wled add client/src/lib/dominantColor.ts client/src/lib/throttle.ts client/src/test/lib && git -C $HOME/github/uber-wled commit -m "client: dominantColor glow helper and throttle for Home v2"`

## Task 5: Client API — Group icon/sortOrder, `reorderGroups`, `ControlPatch.ps` gap-fill

**Files:**
- Modify: `$HOME/github/uber-wled/client/src/api/client.ts` (pre-Phase-D refs: Group at lines 15–19, addGroup/updateGroup at lines 115–119 — Phase D appends/renames below them, so re-locate by symbol)
- Test (Create): `$HOME/github/uber-wled/client/src/test/api/groups.test.ts`
...
- [ ] Gate: `grep -n "applyControl = (targets" $HOME/github/uber-wled/client/src/api/client.ts` — must hit Phase D's v2 fetcher (`export const applyControl = (targets: Target[], patch: ControlPatch) => …`). If instead only the v1 signature `applyControl = (members, action)` exists, STOP — Phase D is incomplete and Phase E depends on it.
...
- [ ] Re-run typecheck: `cd $HOME/github/uber-wled/client && npx tsc -b` — clean.
- [ ] Run: `cd $HOME/github/uber-wled/client && npm test -- src/test/api/groups.test.ts` — all 5 pass.
- [ ] Run full client suite: `cd $HOME/github/uber-wled/client && npm test` — green. (Existing components read only `id`/`name`/`members` off `Group`; untyped test fixtures are unaffected by the added fields.)
- [ ] Commit: `git -C $HOME/github/uber-wled add client/src/api/client.ts client/src/test/api/groups.test.ts && git -C $HOME/github/uber-wled commit -m "client: group icon/sortOrder api, reorderGroups, ControlPatch.ps gap-fill"`

## Task 6: `sections/home/HomeTile.tsx` — tile with glow, quick controls, long-press

**Files:**
- Create: `$HOME/github/uber-wled/client/src/sections/home/HomeTile.tsx`
- Create: `$HOME/github/uber-wled/client/src/sections/home/home.css` (complete stylesheet for the whole phase)
- Test (Create): `$HOME/github/uber-wled/client/src/test/sections/home/HomeTile.test.tsx`
...
- [ ] Run: `cd $HOME/github/uber-wled/client && npm test -- src/test/sections/home/HomeTile.test.tsx` — fails: cannot resolve `../../../sections/home/HomeTile`.
...
- [ ] Run: `cd $HOME/github/uber-wled/client && npm test -- src/test/sections/home/HomeTile.test.tsx` — all 9 pass.
- [ ] Run full client suite: `cd $HOME/github/uber-wled/client && npm test` — green.
- [ ] Commit: `git -C $HOME/github/uber-wled add client/src/sections client/src/test/sections && git -C $HOME/github/uber-wled commit -m "client: HomeTile v2 with dynamic glow, quick controls, long-press"`

## Task 7: `sections/home/HomeSection.tsx` — tile grid, live status, quick-control writes, Control surface

**Files:**
- Create: `$HOME/github/uber-wled/client/src/sections/home/HomeSection.tsx`
- Test (Create): `$HOME/github/uber-wled/client/src/test/sections/home/HomeSection.test.tsx`
...
- [ ] Run: `cd $HOME/github/uber-wled/client && npm test -- src/test/sections/home/HomeSection.test.tsx` — fails: cannot resolve `../../../sections/home/HomeSection`.
...
- [ ] Run: `cd $HOME/github/uber-wled/client && npm test -- src/test/sections/home/HomeSection.test.tsx` — all 8 pass.
- [ ] Run full client suite: `cd $HOME/github/uber-wled/client && npm test` — green.
- [ ] Commit: `git -C $HOME/github/uber-wled add client/src/sections client/src/test/sections && git -C $HOME/github/uber-wled commit -m "client: Home v2 tile grid with live status, glow, quick controls"`

## Task 8: Multi-select mode + floating action bar

**Files:**
- Modify: `$HOME/github/uber-wled/client/src/sections/home/HomeSection.tsx` (from Task 7)
- Test: `$HOME/github/uber-wled/client/src/test/sections/home/HomeSection.test.tsx` (append describe)
...
- [ ] Run: `cd $HOME/github/uber-wled/client && npm test -- src/test/sections/home/HomeSection.test.tsx` — new describe fails (no toolbar rendered).
...
- [ ] Run: `cd $HOME/github/uber-wled/client && npm test -- src/test/sections/home/HomeSection.test.tsx` — all pass (Task 7 tests still green: their tiles receive `selectMode={false}` because no checkbox was clicked).
- [ ] Run full client suite: `cd $HOME/github/uber-wled/client && npm test` — green.
- [ ] Commit: `git -C $HOME/github/uber-wled add client/src/sections/home/HomeSection.tsx client/src/test/sections/home/HomeSection.test.tsx && git -C $HOME/github/uber-wled commit -m "client: Home multi-select with floating action bar into Control surface"`

## Task 9: Edit mode — create / rename / delete rooms with icon picker

**Files:**
- Create: `$HOME/github/uber-wled/client/src/sections/home/IconPicker.tsx`
- Create: `$HOME/github/uber-wled/client/src/sections/home/RoomCreateModal.tsx`
- Create: `$HOME/github/uber-wled/client/src/sections/home/RoomEditTile.tsx`
- Modify: `$HOME/github/uber-wled/client/src/sections/home/HomeSection.tsx`
- Test: `$HOME/github/uber-wled/client/src/test/sections/home/HomeSection.test.tsx` (append describe)
...
- [ ] Run: `cd $HOME/github/uber-wled/client && npm test -- src/test/sections/home/HomeSection.test.tsx` — new describe fails (no `Edit` button).
...
- [ ] Run: `cd $HOME/github/uber-wled/client && npm test -- src/test/sections/home/HomeSection.test.tsx` — all pass.
- [ ] Run full client suite: `cd $HOME/github/uber-wled/client && npm test` — green.
- [ ] Commit: `git -C $HOME/github/uber-wled add client/src/sections/home client/src/test/sections/home && git -C $HOME/github/uber-wled commit -m "client: Home edit mode - create/rename/delete rooms with icon picker"`

## Task 10: Edit mode — room member editor (controller + segment pairs)

**Files:**
- Create: `$HOME/github/uber-wled/client/src/sections/home/RoomMembersEditor.tsx`
- Modify: `$HOME/github/uber-wled/client/src/sections/home/RoomEditTile.tsx`
- Modify: `$HOME/github/uber-wled/client/src/sections/home/HomeSection.tsx` (pass `controllers` + `onMembersChange` into `RoomEditTile`)
- Test: `$HOME/github/uber-wled/client/src/test/sections/home/HomeSection.test.tsx` (append describe)
...
- [ ] Run: `cd $HOME/github/uber-wled/client && npm test -- src/test/sections/home/HomeSection.test.tsx` — new describe fails (no `Members` button).
...
- [ ] Run: `cd $HOME/github/uber-wled/client && npm test -- src/test/sections/home/HomeSection.test.tsx` — all pass.
- [ ] Run full client suite: `cd $HOME/github/uber-wled/client && npm test` — green.
- [ ] Commit: `git -C $HOME/github/uber-wled add client/src/sections/home client/src/test/sections/home && git -C $HOME/github/uber-wled commit -m "client: Home edit mode room member editor with cached segment ids"`

## Task 11: Edit mode — reorder tiles (pointer drag + arrow buttons)

**Files:**
- Create: `$HOME/github/uber-wled/client/src/sections/home/reorder.ts`
- Modify: `$HOME/github/uber-wled/client/src/sections/home/RoomEditTile.tsx`
- Modify: `$HOME/github/uber-wled/client/src/sections/home/HomeSection.tsx`
- Test (Create): `$HOME/github/uber-wled/client/src/test/sections/home/reorder.test.ts`
- Test: `$HOME/github/uber-wled/client/src/test/sections/home/HomeSection.test.tsx` (append describe)
...
- [ ] Run: `cd $HOME/github/uber-wled/client && npm test -- src/test/sections/home/reorder.test.ts src/test/sections/home/HomeSection.test.tsx` — both fail (module missing / no arrow buttons).
...
- [ ] Run: `cd $HOME/github/uber-wled/client && npm test -- src/test/sections/home/reorder.test.ts src/test/sections/home/HomeSection.test.tsx` — all pass.
- [ ] Run full client suite: `cd $HOME/github/uber-wled/client && npm test` — green.
- [ ] Commit: `git -C $HOME/github/uber-wled add client/src/sections/home client/src/test/sections/home && git -C $HOME/github/uber-wled commit -m "client: Home room reordering via drag handle and arrow buttons"`

## Task 12: Wire into AppShell, delete the old Home implementation

**Files:**
- Modify: `$HOME/github/uber-wled/client/src/components/AppShell.tsx` (the Home route import — Phase C's rewritten shell; locate with grep, see step 1)
- Modify: `$HOME/github/uber-wled/client/src/lib/tileStatus.ts` (remove the snapshot-era exports)
- Delete: `$HOME/github/uber-wled/client/src/components/HomeSection.tsx`, `$HOME/github/uber-wled/client/src/components/HomeTile.tsx`, `$HOME/github/uber-wled/client/src/test/HomeSection.test.tsx`, `$HOME/github/uber-wled/client/src/test/components/HomeTile.test.tsx`, `$HOME/github/uber-wled/client/src/test/lib/tileStatus.test.ts`
...
- [ ] Locate the current Home wiring: `grep -rn "HomeSection" $HOME/github/uber-wled/client/src --include='*.tsx' -l` — expect hits in `components/AppShell.tsx` (Phase C shell), old `components/HomeSection.tsx`, and the new section/tests.
...
```
git -C $HOME/github/uber-wled rm client/src/components/HomeSection.tsx client/src/components/HomeTile.tsx client/src/test/HomeSection.test.tsx client/src/test/components/HomeTile.test.tsx client/src/test/lib/tileStatus.test.ts
```
...
- [ ] Commit: `git -C $HOME/github/uber-wled add -A client/src && git -C $HOME/github/uber-wled commit -m "client: Home v2 replaces old HomeSection/HomeTile; remove snapshot tile aggregation"`
- [ ] After phase review passes: `git -C $HOME/github/uber-wled push origin main`
...
- `cd $HOME/github/uber-wled/server && npm test` green
- `cd $HOME/github/uber-wled/client && npm test` green
- `cd $HOME/github/uber-wled/client && npm run build` green
