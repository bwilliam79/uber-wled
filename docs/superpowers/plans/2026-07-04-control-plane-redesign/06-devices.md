...
- [ ] Drift-guard 2 — `grep -n "applyControlV2\|export const applyControl " $HOME/github/uber-wled/client/src/api/client.ts`. Phase E ships `applyControlV2`; if it is missing but Phase D's `applyControl(targets, patch)` exists, append the alias:
