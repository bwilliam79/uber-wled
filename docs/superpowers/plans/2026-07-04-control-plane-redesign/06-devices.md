...
`http://203.0.113.86` (re-verified 2026-07-05: WLED 16.0.0 "Niji", vid 2605030, esp32,
...
- [ ] Run: `cd $HOME/github/uber-wled/client && npm test -- src/test/devices/format.test.ts src/test/devices/segmentLogic.test.ts` → expect FAIL: `Failed to resolve import "../../sections/devices/format"`.
- [ ] Implement:
...
- [ ] Commit: `cd $HOME/github/uber-wled && git add client/src/sections/devices client/src/test/devices && git commit -m "devices: pure uptime/signal formatting and segment validation helpers"`
...
- [ ] Drift-guard 1 — `grep -n "ps?: number" $HOME/github/uber-wled/client/src/api/client.ts`. If the client `ControlPatch` lacks `ps` (Phase D's mirror predates the master update), add it after the `transition` line, verbatim:
...
- [ ] Drift-guard 2 — `grep -n "export const applyControl " $HOME/github/uber-wled/client/src/api/client.ts`. Phase D ships `applyControl(targets, patch)` (Task 1) — this phase consumes that name directly; there is no separate v2-named alias to create. If the grep has no hits, STOP — Phase D is incomplete and this phase depends on it.

- [ ] Drift-guard 3 — `grep -n "listDevicePresets\|interface DevicePreset" $HOME/github/uber-wled/client/src/api/client.ts`. Both must exist (Phase D Task 1). If missing, add:
...
- [ ] Run: `cd $HOME/github/uber-wled/client && npm test -- src/test/devices/apiClient.test.ts` → expect FAIL: `has no exported member 'saveControllerPreset'` (or unresolved import at runtime).
...
- [ ] Run again → expect PASS (11 tests). Also run the whole api + live suites to prove the `LiveInfo` widening broke nothing: `cd $HOME/github/uber-wled/client && npm test -- src/test/api` → PASS.
- [ ] Commit: `cd $HOME/github/uber-wled && git add client/src/api client/src/test/devices && git commit -m "devices: typed api-client functions for config dry-run/apply, reboot, segment CRUD, preset save/delete + device query hooks"`
...
describe('devices fixtures (probed read-only from 203.0.113.86)', () => {
...
- [ ] Run: `cd $HOME/github/uber-wled/client && npm test -- src/test/devices/fixtures.test.ts` → expect FAIL: `Failed to resolve import "./fixtures"`.
- [ ] Create `client/src/test/devices/fixtures.ts`:
...
/**
 * Verbatim read-only probe of http://203.0.113.86/json/cfg (2026-07-05, WLED
...
export const CONTROLLERS: Controller[] = [
  { id: 'c1', name: 'Cabinet Lights', host: '203.0.113.86', source: 'discovered', stale: false, pinnedAssetPattern: 'ESP32' },
...
export const LIVE_INFO: LiveInfo = {
  ...
  fs: { u: 28, t: 983 },
  arch: 'esp32', core: 'v3.3.6', mac: 'c0c3dc112233', ip: '203.0.113.86',
...
- [ ] Commit: `cd $HOME/github/uber-wled && git add client/src/test/devices && git commit -m "devices: shared probed-device fixtures + query/toast test harness"`
...
- [ ] Run: `cd $HOME/github/uber-wled/client && npm test -- src/test/devices/configPatches.test.ts` → expect FAIL: `Failed to resolve import "../../sections/devices/configPatches"`.
- [ ] Create `client/src/sections/devices/configPatches.ts`:
...
- [ ] Commit: `cd $HOME/github/uber-wled && git add client/src/sections/devices client/src/test/devices && git commit -m "devices: pure config patch builders that merge probed cfg rows (order nibble, write-only psk, strand-risk paths)"`
...
- [ ] Run: `cd $HOME/github/uber-wled/client && npm test -- src/test/devices/DiffConfirmModal.test.tsx` → expect FAIL: module not found.
- [ ] Create `client/src/sections/devices/DiffConfirmModal.tsx`:
...
- [ ] Commit: `cd $HOME/github/uber-wled && git add client/src/sections/devices client/src/test/devices && git commit -m "devices: DiffConfirmModal with per-path diff rows, reboot banner, WiFi/GPIO strand-risk gate + section stylesheet"`
...
- [ ] Run: `cd $HOME/github/uber-wled/client && npm test -- src/test/devices/InfoTab.test.tsx` → expect FAIL: module not found.
- [ ] Create `client/src/sections/devices/InfoTab.tsx`:
...
- [ ] Run again → expect PASS (6 tests).
- [ ] Commit: `cd $HOME/github/uber-wled && git add client/src/sections/devices client/src/test/devices && git commit -m "devices: Info tab - facts grid, liveview peek, reboot/remove confirms, native UI link, schedule import"`
...
- [ ] Run: `cd $HOME/github/uber-wled/client && npm test -- src/test/devices/SegmentsTab.test.tsx` → expect FAIL: module not found.
- [ ] Create `client/src/sections/devices/SegmentsTab.tsx`:
...
- [ ] Commit: `cd $HOME/github/uber-wled && git add client/src/sections/devices client/src/test/devices && git commit -m "devices: full segments editor with validated bounds, live toggles, create/delete"`
...
- [ ] Run: `cd $HOME/github/uber-wled/client && npm test -- src/test/devices/DevicePresetsTab.test.tsx` → expect FAIL: module not found.
- [ ] Create `client/src/sections/devices/DevicePresetsTab.tsx`:
...
- [ ] Run again → expect PASS (6 tests).
- [ ] Commit: `cd $HOME/github/uber-wled && git add client/src/sections/devices client/src/test/devices && git commit -m "devices: device presets tab - apply via ps patch, delete confirm, save-current with flags"`
...
- [ ] Run: `cd $HOME/github/uber-wled/client && npm test -- src/test/devices/ConfigTab.test.tsx` → expect FAIL: module not found.
- [ ] Create `client/src/sections/devices/config/types.ts`:
...
- [ ] Run again → expect PASS (6 tests).
- [ ] Commit: `cd $HOME/github/uber-wled && git add client/src/sections/devices client/src/test/devices && git commit -m "devices: Config tab dry-run->diff->confirm pipeline + Identity form"`
...
- [ ] Run: `cd $HOME/github/uber-wled/client && npm test -- src/test/devices/LedHardwareForm.test.tsx` → expect FAIL: module not found.
- [ ] Create `client/src/sections/devices/config/LedHardwareForm.tsx`:
...
- [ ] Run: `cd $HOME/github/uber-wled/client && npm test -- src/test/devices/LedHardwareForm.test.tsx src/test/devices/ConfigTab.test.tsx` → expect PASS (4 + 6 tests, ConfigTab unbroken).
- [ ] Commit: `cd $HOME/github/uber-wled && git add client/src/sections/devices client/src/test/devices && git commit -m "devices: LED & Hardware config form with per-output editor (pin/type/order/skip/auto-white)"`
...
- [ ] Run: `cd $HOME/github/uber-wled/client && npm test -- src/test/devices/WifiForm.test.tsx` → expect FAIL: module not found.
- [ ] Create `client/src/sections/devices/config/WifiForm.tsx`:
...
- [ ] Run: `cd $HOME/github/uber-wled/client && npm test -- src/test/devices/WifiForm.test.tsx src/test/devices/ConfigTab.test.tsx` → expect PASS (4 + 6 tests).
- [ ] Commit: `cd $HOME/github/uber-wled && git add client/src/sections/devices client/src/test/devices && git commit -m "devices: WiFi config form with write-only passwords and static IP validation"`
...
- [ ] Run: `cd $HOME/github/uber-wled/client && npm test -- src/test/devices/configFormsMisc.test.tsx` → expect FAIL: modules not found.
- [ ] Create `client/src/sections/devices/config/SyncForm.tsx`:
...
- [ ] Run: `cd $HOME/github/uber-wled/client && npm test -- src/test/devices/configFormsMisc.test.tsx src/test/devices/ConfigTab.test.tsx` → expect PASS (6 + 6 tests).
- [ ] Commit: `cd $HOME/github/uber-wled && git add client/src/sections/devices client/src/test/devices && git commit -m "devices: Sync, Time, and LED-preferences config forms"`
...
- [ ] Run: `cd $HOME/github/uber-wled/client && npm test -- src/test/devices/AdvancedJsonForm.test.tsx` → expect FAIL: module not found.
- [ ] Create `client/src/sections/devices/config/AdvancedJsonForm.tsx`:
...
- [ ] Run: `cd $HOME/github/uber-wled/client && npm test -- src/test/devices` → expect PASS (whole devices suite so far).
- [ ] Commit: `cd $HOME/github/uber-wled && git add client/src/sections/devices client/src/test/devices && git commit -m "devices: Advanced raw-JSON config editor with parse validation"`
...
- [ ] Run: `cd $HOME/github/uber-wled/client && npm test -- src/test/devices/UpdateTab.test.tsx` → expect FAIL: module not found.
- [ ] Create `client/src/sections/devices/UpdateTab.tsx`:
...
- [ ] Commit: `cd $HOME/github/uber-wled && git add client/src/sections/devices client/src/test/devices && git commit -m "devices: Update tab reusing the firmware pin/OTA flow"`
...
describe('DeviceCard', () => {
  it('shows name, host, version chip and the live metrics', () => {
    ...
    expect(screen.getByText('Cabinet Lights')).toBeTruthy();
    expect(screen.getByText('203.0.113.86')).toBeTruthy();
...
- [ ] Run: `cd $HOME/github/uber-wled/client && npm test -- src/test/devices/DeviceCard.test.tsx` → expect FAIL: module not found.
- [ ] Create `client/src/sections/devices/DeviceCard.tsx`:
...
- [ ] Run again → expect PASS (6 tests).
- [ ] Commit: `cd $HOME/github/uber-wled && git add client/src/sections/devices client/src/test/devices && git commit -m "devices: live-status device card with signal bars, fps, uptime, update badge"`
...
- [ ] Run: `cd $HOME/github/uber-wled/client && npm test -- src/test/devices/route.test.ts src/test/devices/DeviceDetail.test.tsx` → expect FAIL: modules not found.
- [ ] Create `client/src/sections/devices/route.ts`:
...
- [ ] Commit: `cd $HOME/github/uber-wled && git add client/src/sections/devices client/src/test/devices && git commit -m "devices: hash route helpers + DeviceDetail tab shell"`
...
- [ ] Run: `cd $HOME/github/uber-wled/client && npm test -- src/test/devices/DevicesSection.test.tsx` → expect FAIL: module not found.
- [ ] Create `client/src/sections/devices/DevicesSection.tsx`:
...
- [ ] Run again → expect PASS (6 tests). Also run the whole devices suite: `cd $HOME/github/uber-wled/client && npm test -- src/test/devices` → PASS.
- [ ] Commit: `cd $HOME/github/uber-wled && git add client/src/sections/devices client/src/test/devices && git commit -m "devices: DevicesSection list + detail routing, add-controller modal, Control surface wiring"`
...
- [ ] Run: `cd $HOME/github/uber-wled/client && npm test -- src/test/AppShell.test.tsx` → expect FAIL: `AppShell.tsx` has no exported member `sectionFromHash` (and, once exported, the deep link resolves to `home` because the old implementation matches the whole hash string).
...
- [ ] If any existing AppShell test asserted old Controllers-screen content (e.g. the `Controllers` heading) for the devices route, update that assertion to the new list heading `Devices` — check with `grep -n "Controllers" $HOME/github/uber-wled/client/src/test/AppShell.test.tsx`.
- [ ] Run: `cd $HOME/github/uber-wled/client && npm test -- src/test/AppShell.test.tsx` → expect PASS (Phase C's suite + 2 new tests).
- [ ] Delete the replaced files:

```
git -C $HOME/github/uber-wled rm client/src/components/ControllersSection.tsx client/src/components/ControllerList.tsx client/src/test/ControllersSection.test.tsx client/src/test/ControllerList.test.tsx
```

- [ ] Verify zero dangling references: `grep -rn "ControllersSection\|ControllerList" $HOME/github/uber-wled/client/src --include='*.ts*'` → expect no hits. Then confirm the kept firmware components still have their consumers: `grep -rln "FirmwareStatus" $HOME/github/uber-wled/client/src/components $HOME/github/uber-wled/client/src/sections` → expect `FirmwareSection.tsx` (until Phase H) and `sections/devices/UpdateTab.tsx`.
...
- [ ] Commit: `cd $HOME/github/uber-wled && git add -A client/src && git commit -m "devices: AppShell renders DevicesSection, first-segment hash routing; delete old Controllers screen"`
...
- [ ] Full suites and build, all green:
  - `cd $HOME/github/uber-wled/server && npm test`
  - `cd $HOME/github/uber-wled/client && npm test`
  - `cd $HOME/github/uber-wled/client && npm run build`
- [ ] Contract greps (each must hit):
  - Phase H firmware-hook drift target: `grep -n "queryKey: \['firmware', controllerId\]" $HOME/github/uber-wled/client/src/api/queries.ts`
  - Routing contract: `grep -rn "#/devices" $HOME/github/uber-wled/client/src/sections/devices/route.ts $HOME/github/uber-wled/client/src/sections/devices/DevicesSection.tsx` and `grep -n "split('/')\[0\]" $HOME/github/uber-wled/client/src/components/AppShell.tsx`
  - Preset apply transport (no dedicated route): `grep -n "applyControl" $HOME/github/uber-wled/client/src/sections/devices/DevicePresetsTab.tsx` and `grep -rn "presets/apply" $HOME/github/uber-wled/client/src` → the second must have NO hits.
  - No version bumps slipped in: `git -C $HOME/github/uber-wled diff main --stat -- client/package.json server/package.json` shows no `"version"` change (1.0.0 happens in Phase I).
- [ ] Browser walkthrough against the dev server with the real controller (start with `cd $HOME/github/uber-wled && npm run dev` or the repo's documented dev command), at BOTH 390px and 1440px widths. **Hardware policy for this walkthrough** — reads are unrestricted; the ONLY permitted write is the reversible segment power toggle below; NEVER confirm a config diff, save/delete a device preset, reboot, or OTA-update the real device:
...
- [ ] Fix anything the walkthrough surfaces; re-run the failing suite; commit fixes: `cd $HOME/github/uber-wled && git add -A client && git commit -m "devices: phase F verification fixes"` (skip if nothing changed).
