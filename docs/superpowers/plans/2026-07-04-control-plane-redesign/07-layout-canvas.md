...
Test commands: all client — `cd $HOME/github/uber-wled/client && npm test -- <file>`. There are no server tasks in this phase.
...
- [ ] Run it and confirm it fails with "Cannot find module '../../../sections/layout/geometry'":
  `cd $HOME/github/uber-wled/client && npm test -- src/test/sections/layout/geometry.test.ts`
- [ ] Create `client/src/sections/layout/geometry.ts`:
...
- [ ] Run again, expect all green:
  `cd $HOME/github/uber-wled/client && npm test -- src/test/sections/layout/geometry.test.ts`
- [ ] Commit:
  `cd $HOME/github/uber-wled && git add client/src/sections/layout/geometry.ts client/src/test/sections/layout/geometry.test.ts && git commit -m "layout: pure geometry helpers (transforms, snapping, hit tests, fit-all)"`
...
- [ ] Run and confirm module-not-found failure:
  `cd $HOME/github/uber-wled/client && npm test -- src/test/sections/layout/canvasMode.test.ts`
- [ ] Create `client/src/sections/layout/canvasMode.ts`:
...
- [ ] Run again, expect green:
  `cd $HOME/github/uber-wled/client && npm test -- src/test/sections/layout/canvasMode.test.ts`
- [ ] Commit:
  `cd $HOME/github/uber-wled && git add client/src/sections/layout/canvasMode.ts client/src/test/sections/layout/canvasMode.test.ts && git commit -m "layout: typed mode reducer for draw/edit/pan/marquee state machine"`
...
- [ ] Write the failing test `client/src/test/sections/layout/stripColors.test.ts`. The segment fixtures are real data captured 2026-07-04 from the controller at 203.0.113.86 (WLED 16.0.0 "Niji", `GET /json/state`):

```ts
import { describe, it, expect } from 'vitest';
import { stripStrokeColor, OFFLINE_STROKE, type LiveControllerStatus } from '../../../sections/layout/stripColors';

// Real segments from 203.0.113.86 /json/state (2026-07-04): a white RGBW segment
...
- [ ] Run and confirm module-not-found failure:
  `cd $HOME/github/uber-wled/client && npm test -- src/test/sections/layout/stripColors.test.ts`
- [ ] Create `client/src/sections/layout/stripColors.ts`:
...
- [ ] Run and confirm it fails (missing exports):
  `cd $HOME/github/uber-wled/client && npm test -- src/test/api/queries.layout.test.tsx`
...
- [ ] Run again, expect green:
  `cd $HOME/github/uber-wled/client && npm test -- src/test/api/queries.layout.test.tsx`
- [ ] Commit:
  `cd $HOME/github/uber-wled && git add client/src/sections/layout/stripColors.ts client/src/api/queries.ts client/src/test/sections/layout/stripColors.test.ts client/src/test/api/queries.layout.test.tsx && git commit -m "layout: live stroke color helper and strips/room-labels query hooks"`
...
- [ ] Run and confirm module-not-found failure:
  `cd $HOME/github/uber-wled/client && npm test -- src/test/sections/layout/RoomLabels.test.tsx`
- [ ] Create `client/src/sections/layout/RoomLabels.tsx`:
...
- [ ] Run again, expect green:
  `cd $HOME/github/uber-wled/client && npm test -- src/test/sections/layout/RoomLabels.test.tsx`
- [ ] Commit:
  `cd $HOME/github/uber-wled && git add client/src/sections/layout/RoomLabels.tsx client/src/test/sections/layout/RoomLabels.test.tsx && git commit -m "layout: room label chips with drag and inline rename"`
...
// Real segment data captured 2026-07-04 from 203.0.113.86 /json/state.
...
- [ ] Run and confirm module-not-found failure:
  `cd $HOME/github/uber-wled/client && npm test -- src/test/sections/layout/LayoutCanvas.test.tsx`
- [ ] Create `client/src/sections/layout/LayoutCanvas.tsx`:
...
- [ ] Run again, expect green:
  `cd $HOME/github/uber-wled/client && npm test -- src/test/sections/layout/LayoutCanvas.test.tsx`
- [ ] Commit:
  `cd $HOME/github/uber-wled && git add client/src/sections/layout/LayoutCanvas.tsx client/src/test/sections/layout/LayoutCanvas.test.tsx && git commit -m "layout: SVG canvas renderer with viewport transform, live strokes, glow, handles"`
...
- [ ] Run and confirm it fails with "Cannot find module '../../../sections/layout/LayoutSection'":
  `cd $HOME/github/uber-wled/client && npm test -- src/test/sections/layout/LayoutSection.test.tsx`
...
- [ ] Run the integration tests, expect all green:
  `cd $HOME/github/uber-wled/client && npm test -- src/test/sections/layout/LayoutSection.test.tsx`
- [ ] Commit:
  `cd $HOME/github/uber-wled && git add client/src/sections/layout/LayoutSection.tsx client/src/sections/layout/layout.css client/src/test/sections/layout/LayoutSection.test.tsx && git commit -m "layout: rebuilt LayoutSection with draw/edit/pan/marquee and ControlSurface handoff"`
...
- Kept deliberately: `client/src/lib/segmentColor.ts` + `client/src/test/lib/segmentColor.test.ts` (consumed by `stripColors.ts` and by Home), `client/src/components/ControlPanel.tsx` (still imported by the old HomeSection until Phase E deletes it — verify with grep before touching).
...
- [ ] Verify nothing else imports the legacy modules (expect only the four files being deleted and their tests):
  `grep -rn "StripCanvas\|StripPathEditor\|RoomLabelLayer\|components/LayoutSection" $HOME/github/uber-wled/client/src`
- [ ] Delete the legacy implementation and test files:
  `cd $HOME/github/uber-wled && git rm client/src/components/LayoutSection.tsx client/src/components/StripCanvas.tsx client/src/components/StripPathEditor.tsx client/src/components/RoomLabelLayer.tsx client/src/test/LayoutSection.test.tsx client/src/test/components/StripCanvas.test.tsx client/src/test/components/StripPathEditor.test.tsx client/src/test/components/RoomLabelLayer.test.tsx`
...
- [ ] Run the full client suite and the production build — both must be green (the suite proves no test still references the deleted files; the build proves no dangling imports):
  `cd $HOME/github/uber-wled/client && npm test && npm run build`
- [ ] Commit:
  `cd $HOME/github/uber-wled && git add -A client/src && git commit -m "layout: route to rebuilt canvas, delete legacy StripCanvas/StripPathEditor/RoomLabelLayer"`

---

## Phase completion checklist

- [ ] `cd $HOME/github/uber-wled/client && npm test` — full suite green.
- [ ] `cd $HOME/github/uber-wled/client && npm run build` — green.
- [ ] `cd $HOME/github/uber-wled/server && npm test` — still green (this phase must not have touched the server).
