...
- [ ] Check what Phase D already shipped:
  ```bash
  grep -n "applyControlV2\|ControllerCapabilities\|getCapabilities" $HOME/github/uber-wled/client/src/api/client.ts
  ls $HOME/github/uber-wled/client/src/api/queries.ts $HOME/github/uber-wled/client/src/control/ColorWheel.tsx $HOME/github/uber-wled/client/src/test/renderWithQuery.tsx 2>&1
  ```
...
- [ ] Write the failing contract test `client/src/test/api/phaseHContracts.test.ts`:
  ...
  ```
- [ ] Run it: `cd $HOME/github/uber-wled/client && npm test -- phaseHContracts` — expect FAIL (`applyControlV2`/`getCapabilities` not exported) unless Phase D shipped them, in which case expect PASS and skip the next step.
