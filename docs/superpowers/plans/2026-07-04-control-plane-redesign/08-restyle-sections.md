...
- [ ] Gate on prerequisite phases. Run:
  ```bash
  ls $HOME/github/uber-wled/client/src/components/ui/Button.tsx \
     $HOME/github/uber-wled/client/src/design/tokens.css
  grep -c '"@tanstack/react-query"' $HOME/github/uber-wled/client/package.json
  ```
...
- [ ] Check what Phase D already shipped:
  ```bash
  grep -n "export const applyControl \|ControllerCapabilities\|getCapabilities" $HOME/github/uber-wled/client/src/api/client.ts
  ls $HOME/github/uber-wled/client/src/api/queries.ts $HOME/github/uber-wled/client/src/components/ui/ColorWheel.tsx $HOME/github/uber-wled/client/src/test/renderWithQuery.tsx 2>&1
  ```
...
- [ ] Write the failing contract test `client/src/test/api/phaseHContracts.test.ts`:
  ...
  ```
- [ ] Run it: `cd $HOME/github/uber-wled/client && npm test -- phaseHContracts` — expect FAIL (`applyControl`/`getCapabilities` not exported) unless Phase D shipped them, in which case expect PASS and skip the next step.
...
- [ ] Create `client/src/test/renderWithQuery.tsx` ONLY if missing:
  ...
  ```
- [ ] Run: `cd $HOME/github/uber-wled/client && npm test -- phaseHContracts` — expect PASS (2 tests).
- [ ] Run the whole client suite to prove nothing regressed: `cd $HOME/github/uber-wled/client && npm test` — expect PASS.
- [ ] Commit:
  ```bash
  cd $HOME/github/uber-wled && git add client/src && git commit -m "Phase H Task 1: pin client contracts for capabilities + control fan-out v2"
  ```
...
- [ ] Write failing test `client/src/test/lib/color.test.ts`:
  ...
  ```
- [ ] Write failing test `client/src/test/lib/paletteCss.test.ts` (stop data is REAL `/json/palx` output captured from 203.0.113.86, WLED 16.0.0 vid 2605030):
  ```ts
  import { describe, it, expect } from 'vitest';
  ...
  ```
- [ ] Run: `cd $HOME/github/uber-wled/client && npm test -- test/lib/color test/lib/paletteCss` — expect FAIL (modules missing).
- [ ] Create `client/src/lib/color.ts`:
  ```ts
...
- [ ] Create `client/src/lib/paletteCss.ts`:
  ...
  ```
- [ ] Run: `cd $HOME/github/uber-wled/client && npm test -- test/lib/color test/lib/paletteCss` — expect PASS (6 tests).
- [ ] Commit:
  ```bash
  cd $HOME/github/uber-wled && git add client/src/lib client/src/test/lib && git commit -m "Phase H Task 2: color + palette gradient helpers"
  ```
...
- [ ] Create the shared fixture `client/src/test/fixtures/capabilities.ts` (trimmed REAL data captured from http://203.0.113.86 — effect/palette names from `/json/eff` + `/json/pal`, preview stops from `/json/palx?page=0`, vid from `/json/info`):
  ```ts
  ...
  /** Trimmed real capability data from 203.0.113.86 (WLED 16.0.0 "Niji", vid 2605030). */
...
- [ ] Write failing test `client/src/test/ThemePickers.test.tsx`:
  ...
  ```
- [ ] Run: `cd $HOME/github/uber-wled/client && npm test -- ThemePickers` — expect FAIL (components missing).
- [ ] Create `client/src/sections/themes/EffectPicker.tsx`:
  ```tsx
...
- [ ] Run: `cd $HOME/github/uber-wled/client && npm test -- ThemePickers` — expect PASS (4 tests).
- [ ] Commit:
  ```bash
  cd $HOME/github/uber-wled && git add client/src/sections/themes client/src/test && git commit -m "Phase H Task 3: themes effect + palette pickers with gradient previews"
  ```
...
- [ ] Write failing test `client/src/test/ThemeForm.test.tsx`:
  ...
  ```
- [ ] Run: `cd $HOME/github/uber-wled/client && npm test -- ThemeForm` — expect FAIL (components missing).
- [ ] Create `client/src/sections/themes/ColorSlotButton.tsx`:
  ```tsx
...
- [ ] Create `client/src/sections/themes/ThemeForm.tsx`:
  ...
  ```
- [ ] Run: `cd $HOME/github/uber-wled/client && npm test -- ThemeForm` — expect PASS (2 tests).
- [ ] Commit:
  ```bash
  cd $HOME/github/uber-wled && git add client/src/sections/themes client/src/test/ThemeForm.test.tsx && git commit -m "Phase H Task 4: theme form with color-slot popovers and payload assembly"
  ```
...
- [ ] Write failing test `client/src/test/ThemesSection.test.tsx`:
  ...
  ```
- [ ] Run: `cd $HOME/github/uber-wled/client && npm test -- ThemesSection` — expect FAIL (component missing).
- [ ] Create `client/src/sections/themes/ThemesSection.tsx`:
  ```tsx
  import { useMemo, useState } from 'react';
  import { useMutation, useQueryClient } from '@tanstack/react-query';
...
- [ ] Create `client/src/sections/themes/themes.css`:
  ...
  ```
- [ ] Run: `cd $HOME/github/uber-wled/client && npm test -- ThemesSection` — expect PASS (2 tests).
...
- [ ] Run the full client suite and build: `cd $HOME/github/uber-wled/client && npm test && npm run build` — expect PASS.
- [ ] Commit:
  ```bash
  cd $HOME/github/uber-wled && git add -A client/src && git commit -m "Phase H Task 5: ThemesSection v2 with capability-cache pickers; delete ThemeManager"
  ```
...
- [ ] Rewrite `client/src/test/CalendarGrid.test.tsx` to target the new path (failing first):
  ...
  ```
- [ ] Run: `cd $HOME/github/uber-wled/client && npm test -- CalendarGrid` — expect FAIL (new module missing).
- [ ] Create `client/src/sections/schedule/CalendarGrid.tsx`:
  ```tsx
  import type { CalendarEvent } from '../../api/client';
...
- [ ] Create `client/src/sections/schedule/schedule.css`:
  ...
  ```
- [ ] Run: `cd $HOME/github/uber-wled/client && npm test -- CalendarGrid` — expect PASS (3 tests). (The old `components/CalendarGrid.tsx` still exists and is still imported by the old `ScheduleSection`; both are deleted in Task 9.)
- [ ] Commit:
  ```bash
  cd $HOME/github/uber-wled && git add client/src/sections/schedule client/src/test/CalendarGrid.test.tsx && git commit -m "Phase H Task 6: restyled CalendarGrid on the UI kit"
  ```
...
- [ ] Rewrite `client/src/test/WeeklyScheduleForm.test.tsx` (failing first):
  ...
  ```
- [ ] Run: `cd $HOME/github/uber-wled/client && npm test -- CalendarEventForm WeeklyScheduleForm` — expect FAIL (new modules missing).
- [ ] Create `client/src/sections/schedule/CalendarEventForm.tsx`:
  ```tsx
  import { useState } from 'react';
  import {
...
- [ ] Create `client/src/sections/schedule/WeeklyScheduleForm.tsx`:
  ...
  ```
- [ ] Run: `cd $HOME/github/uber-wled/client && npm test -- CalendarEventForm WeeklyScheduleForm` — expect PASS (4 tests).
- [ ] Commit:
  ```bash
  cd $HOME/github/uber-wled && git add client/src/sections/schedule client/src/test && git commit -m "Phase H Task 7: restyled calendar event + weekly schedule forms"
  ```
...
- [ ] Write failing test `client/src/test/ScheduleManager.test.tsx`:
  ...
  ```
- [ ] Run: `cd $HOME/github/uber-wled/client && npm test -- test/ScheduleManager` — expect FAIL (module missing).
- [ ] Create `client/src/sections/schedule/ScheduleManager.tsx`:
  ```tsx
  import { useState } from 'react';
  ...
  ```
- [ ] Run: `cd $HOME/github/uber-wled/client && npm test -- test/ScheduleManager` — expect PASS (3 tests).
- [ ] Commit:
  ```bash
  cd $HOME/github/uber-wled && git add client/src/sections/schedule client/src/test/ScheduleManager.test.tsx && git commit -m "Phase H Task 8: ScheduleManager preview/revert on control fan-out v2"
  ```
...
- [ ] Rewrite `client/src/test/ScheduleSection.test.tsx` (failing first):
  ...
  ```
- [ ] Run: `cd $HOME/github/uber-wled/client && npm test -- test/ScheduleSection` — expect FAIL (new module missing).
- [ ] Create `client/src/sections/schedule/ScheduleSection.tsx`:
  ```tsx
  import { useState } from 'react';
  import { useQueryClient } from '@tanstack/react-query';
  ...
  ```
- [ ] Run: `cd $HOME/github/uber-wled/client && npm test -- test/ScheduleSection` — expect PASS (3 tests).
...
- [ ] Commit:
  ```bash
  cd $HOME/github/uber-wled && git add -A client/src && git commit -m "Phase H Task 9: ScheduleSection v2 assembled; delete old schedule components"
  ```
...
- [ ] Rewrite `client/src/test/FirmwareSection.test.tsx` (failing first):
  ...
  ```
- [ ] Run: `cd $HOME/github/uber-wled/client && npm test -- FirmwareSection` — expect FAIL (new module missing).
- [ ] Create `client/src/sections/firmware/FirmwareSection.tsx`:
  ```tsx
...
- [ ] Create `client/src/sections/firmware/firmware.css`:
  ...
  ```
- [ ] Run: `cd $HOME/github/uber-wled/client && npm test -- FirmwareSection` — expect PASS (2 tests).
...
- [ ] Delete the old fleet component: `cd $HOME/github/uber-wled && git rm client/src/components/FirmwareSection.tsx`. Leave `FirmwareStatus.tsx`, `AssetPickerModal.tsx`, and their tests in place (consumed by Phase F's Devices → Update tab).
- [ ] Run the full client suite and build: `cd $HOME/github/uber-wled/client && npm test && npm run build` — expect PASS.
- [ ] Commit:
  ```bash
  cd $HOME/github/uber-wled && git add -A client/src && git commit -m "Phase H Task 10: Firmware fleet view on the kit with Devices Update deep-link"
  ```
...
- [ ] Rewrite `client/src/test/SettingsSection.test.tsx` (failing first):
  ...
  ```
- [ ] Run: `cd $HOME/github/uber-wled/client && npm test -- SettingsSection` — expect FAIL (new module missing).
- [ ] Create `client/src/sections/settings/SettingsSection.tsx`:
  ```tsx
...
- [ ] Create `client/src/sections/settings/settings.css`:
  ...
  ```
- [ ] Run: `cd $HOME/github/uber-wled/client && npm test -- SettingsSection` — expect PASS (4 tests).
...
- [ ] Delete the old component: `cd $HOME/github/uber-wled && git rm client/src/components/SettingsSection.tsx`.
- [ ] Run the full client suite and build: `cd $HOME/github/uber-wled/client && npm test && npm run build` — expect PASS.
- [ ] Commit:
  ```bash
  cd $HOME/github/uber-wled && git add -A client/src && git commit -m "Phase H Task 11: Settings on the kit with live poll interval field"
  ```
...
- [ ] Full client suite: `cd $HOME/github/uber-wled/client && npm test` — expect PASS, zero skips.
- [ ] Client production build: `cd $HOME/github/uber-wled/client && npm run build` — expect success (tsc + vite).
- [ ] Full server suite (untouched by this phase, but the gate requires it): `cd $HOME/github/uber-wled/server && npm test` — expect PASS.
- [ ] Manual width check with the dev server (`cd $HOME/github/uber-wled/client && npm run dev`): open Themes, Schedule, Firmware, Settings at 390px and 1440px viewport widths; verify no horizontal scrolling, picker lists scroll internally, color popover stays on-screen at 390px, all touch targets ≥ 40px. This is a look-only check against real controllers — do NOT save themes/schedules or trigger updates against real hardware during it (read-only browsing is fine).
- [ ] If any fix was needed, commit it:
  ```bash
  cd $HOME/github/uber-wled && git add -A client/src && git commit -m "Phase H Task 12: verification fixes"
  ```
