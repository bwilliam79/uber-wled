...
- Repo root: `$HOME/github/uber-wled`. Client tests run as
  `cd $HOME/github/uber-wled/client && npm test -- <file>`.
...
```
cd $HOME/github/uber-wled/client && npm test -- src/test/api/queryClient.test.ts
```

- [ ] Install the three new deps (exact names from the master contract; no others):

```
cd $HOME/github/uber-wled/client && npm install @tanstack/react-query @jaames/iro @fontsource/plus-jakarta-sans
```
...
```
grep -rn "googleapis\|fonts.g" $HOME/github/uber-wled/client/src $HOME/github/uber-wled/client/index.html ; echo "exit=$?"
...
```
cd $HOME/github/uber-wled && git add client && git commit -m "client: add react-query/iro/fontsource deps, self-host Plus Jakarta Sans, wire QueryClientProvider"
```
...
```
cd $HOME/github/uber-wled/client && npm test -- src/test/design/tokens.test.ts
```
...
```
cd $HOME/github/uber-wled && git add client && git commit -m "client: add design tokens + global styles, remap legacy CSS vars onto new palette"
```
...
```
cd $HOME/github/uber-wled/client && npm test -- src/test/components/ui/ui-basics.test.tsx
```
...
cd $HOME/github/uber-wled && git add client && git commit -m "client: UI kit part 1 (Button, IconButton, Card, Chip, Field, Skeleton) + nav/kit icons"
```
...
```
cd $HOME/github/uber-wled/client && npm test -- src/test/components/ui/Slider.test.tsx src/test/components/ui/Toggle.test.tsx
```
...
cd $HOME/github/uber-wled/client && npm test -- src/test/components/ui/Slider.test.tsx src/test/components/ui/Toggle.test.tsx
cd $HOME/github/uber-wled/client && npm test
cd $HOME/github/uber-wled && git add client && git commit -m "client: UI kit Slider (40px touch target, colored fill, commit events) + Toggle"
```
...
```
cd $HOME/github/uber-wled/client && npm test -- src/test/components/ui/Tabs.test.tsx src/test/components/ui/pickers.test.tsx
```
...
cd $HOME/github/uber-wled/client && npm test -- src/test/components/ui/Tabs.test.tsx src/test/components/ui/pickers.test.tsx
cd $HOME/github/uber-wled/client && npm test
cd $HOME/github/uber-wled && git add client && git commit -m "client: UI kit Tabs, SegmentedControl, SearchInput, Select with keyboard + aria semantics"
```
...
```
cd $HOME/github/uber-wled/client && npm test -- src/test/components/ui/Modal.test.tsx src/test/components/ui/Drawer.test.tsx
```
...
cd $HOME/github/uber-wled/client && npm test -- src/test/components/ui/Modal.test.tsx src/test/components/ui/Drawer.test.tsx
cd $HOME/github/uber-wled/client && npm test
cd $HOME/github/uber-wled && git add client && git commit -m "client: UI kit Modal (focus trap, Esc/overlay close) + Drawer (slide-over / bottom sheet)"
```
...
```
cd $HOME/github/uber-wled/client && npm test -- src/test/components/ui/Toast.test.tsx
```
...
cd $HOME/github/uber-wled/client && npm test -- src/test/components/ui/Toast.test.tsx
cd $HOME/github/uber-wled/client && npm test
cd $HOME/github/uber-wled && git add client && git commit -m "client: UI kit Toast provider with useToast(), auto-dismiss, actions"
```
...
```
cd $HOME/github/uber-wled/client && npm test -- src/test/AppShell.test.tsx
```
...
```
cd $HOME/github/uber-wled && git add client && git commit -m "client: AppShell v2 — 7-section IA, Devices replaces Controllers, bottom nav <900px, react-query firmware badge"
```
...
```
grep -rn "googleapis\|fonts.gstatic" $HOME/github/uber-wled/client/src $HOME/github/uber-wled/client/index.html $HOME/github/uber-wled/design-system ; echo "exit=$?"
```
...
```
cd $HOME/github/uber-wled && git add design-system && git commit -m "design-system: rewrite MASTER.md for sleek smart-home tokens, self-hosted type, ui kit contract"
```
