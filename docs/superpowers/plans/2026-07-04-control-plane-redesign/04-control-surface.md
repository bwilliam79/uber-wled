...
2 segments) at `http://203.0.113.86` on 2026-07-04 via read-only GETs
...
- [ ] Run it and confirm the failure mode: `cd $HOME/github/uber-wled/client && npm test -- src/test/api/clientV2.test.ts` — expect FAIL: `client.ts` has no exports `applyControlV1`, `getCapabilities`, `listDevicePresets`, `Target`, `ControlPatch`; and `applyControl` has the wrong signature.
...
- [ ] Run the new test: `cd $HOME/github/uber-wled/client && npm test -- src/test/api/clientV2.test.ts` — expect PASS (6 tests).
- [ ] Run the whole client suite to catch rename fallout: `cd $HOME/github/uber-wled/client && npm test` — expect PASS (existing HomeSection/LayoutSection/ScheduleSection tests mock `fetch`, not the fetcher, so only compile errors would surface).
- [ ] Commit: `cd $HOME/github/uber-wled && git add client && git commit -m "Phase D task 1: mirror v2 control types on client, add applyControl(targets, patch), rename legacy fetcher to applyControlV1"`
...
- [ ] Ensure the dependency exists (Phase C installs it; this is idempotent): `cd $HOME/github/uber-wled/client && ls node_modules/@tanstack/react-query >/dev/null 2>&1 || npm install @tanstack/react-query`
...
- [ ] Run: `cd $HOME/github/uber-wled/client && npm test -- src/test/api/queries.test.tsx` — expect FAIL: `Cannot find module '../../api/queries'`.
- [ ] Create `client/src/api/queries.ts`:
...
- [ ] Run: `cd $HOME/github/uber-wled/client && npm test -- src/test/api/queries.test.tsx` — expect PASS (4 tests).
- [ ] Run full suite: `cd $HOME/github/uber-wled/client && npm test` — expect PASS.
- [ ] Commit: `cd $HOME/github/uber-wled && git add client && git commit -m "Phase D task 2: react-query hooks with master query keys (controllers/groups/themes/status/capabilities/presets)"`
...
- [ ] Run: `cd $HOME/github/uber-wled/client && npm test -- src/test/api/live.test.tsx` — expect FAIL: `Cannot find module '../../api/live'`.
- [ ] Create `client/src/api/live.ts`:
...
- [ ] Run: `cd $HOME/github/uber-wled/client && npm test -- src/test/api/live.test.tsx` — expect PASS (6 tests).
- [ ] Commit: `cd $HOME/github/uber-wled && git add client && git commit -m "Phase D task 3: useLiveStatus SSE hook with capped backoff and stable-key resubscription"`
...
- [ ] Run: `cd $HOME/github/uber-wled/client && npm test -- src/test/lib/color.test.ts` — expect FAIL: module not found.
...
- [ ] Run: `cd $HOME/github/uber-wled/client && npm test -- src/test/lib/color.test.ts` — expect PASS.
...
- [ ] Run: `cd $HOME/github/uber-wled/client && npm test -- src/test/lib/recentColors.test.ts` — expect FAIL: module not found.
...
- [ ] Run: `cd $HOME/github/uber-wled/client && npm test -- src/test/lib/recentColors.test.ts` — expect PASS.
...
- [ ] Run: `cd $HOME/github/uber-wled/client && npm test -- src/test/lib/throttle.test.ts` — expect FAIL: module not found.
...
- [ ] Run all three: `cd $HOME/github/uber-wled/client && npm test -- src/test/lib` — expect PASS (existing lib tests included).
- [ ] Commit: `cd $HOME/github/uber-wled && git add client && git commit -m "Phase D task 4: pure helpers - kelvinToRgb, hex/rgb, palette gradient css, recent colors, trailing throttle"`
...
// ── FxMeta fixtures ─────────────────────────────────────────────────────────
// Captured from WLED 16.0.0 (vid 2605030) at 203.0.113.86 on 2026-07-04.
...
- [ ] Run: `cd $HOME/github/uber-wled/client && npm test -- src/test/control/controlState.test.ts` — expect FAIL: `Cannot find module '../../control/controlState'`.
...
- [ ] Run: `cd $HOME/github/uber-wled/client && npm test -- src/test/control/controlState.test.ts` — expect PASS (all describe blocks).
- [ ] Commit: `cd $HOME/github/uber-wled && git add client && git commit -m "Phase D task 5: pure control-state aggregation, target expansion, capability merge with real-device fixtures"`
...
- [ ] Ensure the dependency exists: `cd $HOME/github/uber-wled/client && ls node_modules/@jaames/iro >/dev/null 2>&1 || npm install @jaames/iro`
...
- [ ] Run: `cd $HOME/github/uber-wled/client && npm test -- src/test/components/ColorWheel.test.tsx` — expect FAIL: `Cannot find module '../../components/ui/ColorWheel'`.
- [ ] Create `client/src/components/ui/ColorWheel.tsx`:
...
- [ ] Run: `cd $HOME/github/uber-wled/client && npm test -- src/test/components/ColorWheel.test.tsx` — expect PASS (4 tests).
- [ ] Commit: `cd $HOME/github/uber-wled && git add client && git commit -m "Phase D task 6: controlled iro.js ColorWheel wrapper"`
...
- [ ] Run: `cd $HOME/github/uber-wled/client && npm test -- src/test/control/ColorTab.test.tsx` — expect FAIL: module not found.
- [ ] Create `client/src/control/ColorTab.tsx`:
...
- [ ] Run: `cd $HOME/github/uber-wled/client && npm test -- src/test/control/ColorTab.test.tsx` — expect PASS (7 tests).
- [ ] Commit: `cd $HOME/github/uber-wled && git add client && git commit -m "Phase D task 7: ColorTab with effect-driven slots, hex/RGB/white/CCT, kelvin chips, recent colors"`
...
- [ ] Run: `cd $HOME/github/uber-wled/client && npm test -- src/test/control/EffectsTab.test.tsx` — expect FAIL: module not found.
- [ ] Create `client/src/control/EffectsTab.tsx`:
...
- [ ] Run: `cd $HOME/github/uber-wled/client && npm test -- src/test/control/EffectsTab.test.tsx` — expect PASS (6 tests).
- [ ] Commit: `cd $HOME/github/uber-wled && git add client && git commit -m "Phase D task 8: EffectsTab with fxdata-driven dynamic controls and capability badges"`
...
- [ ] Run: `cd $HOME/github/uber-wled/client && npm test -- src/test/control/PalettesTab.test.tsx` — expect FAIL: module not found.
- [ ] Create `client/src/control/PalettesTab.tsx`:
...
- [ ] Run: `cd $HOME/github/uber-wled/client && npm test -- src/test/control/PalettesTab.test.tsx` — expect PASS (5 tests).
- [ ] Commit: `cd $HOME/github/uber-wled && git add client && git commit -m "Phase D task 9: PalettesTab with real gradient previews, random badge, slot-color rendering"`
...
- [ ] Run: `cd $HOME/github/uber-wled/client && npm test -- src/test/control/PresetsTab.test.tsx` — expect FAIL: module not found.
- [ ] Create `client/src/control/PresetsTab.tsx`:
...
- [ ] Run: `cd $HOME/github/uber-wled/client && npm test -- src/test/control/PresetsTab.test.tsx` — expect PASS (4 tests).
- [ ] Commit: `cd $HOME/github/uber-wled && git add client && git commit -m "Phase D task 10: PresetsTab - themes always, device presets single-controller only"`
...
- [ ] Run: `cd $HOME/github/uber-wled/client && npm test -- src/test/control/ControlSurface.test.tsx` — expect FAIL: `Cannot find module '../../control/ControlSurface'`.
...
- [ ] Run: `cd $HOME/github/uber-wled/client && npm test -- src/test/control/ControlSurface.test.tsx` — expect PASS (9 tests). If a kit prop mismatch surfaces here (e.g. `Toggle` renders a `button role="switch"` instead of a checkbox), fix the ControlSurface call site / test query per the kit-contract preflight — never the kit semantics.
- [ ] Run the full client suite and build: `cd $HOME/github/uber-wled/client && npm test && npm run build` — expect both green.
- [ ] Commit: `cd $HOME/github/uber-wled && git add client && git commit -m "Phase D task 11: ControlSurface with header controls, 4 tabs, optimistic throttled writes, partial-failure toast"`

---

## Phase completion gate

- `cd $HOME/github/uber-wled/client && npm test` — all green.
- `cd $HOME/github/uber-wled/client && npm run build` — green.
- `cd $HOME/github/uber-wled/server && npm test` — green (this phase touches no server code; run to prove it).
- No version bumps in this phase: master reserves `1.0.0` for Phase I.
- Optional live smoke (state-level only, per hardware policy): with the dev servers running and a real controller registered, open the Control surface, capture the device state via `GET http://203.0.113.86/json/state`, change brightness once, verify the SSE-driven UI reflects it, then restore the captured `on`/`bri` values through the same UI. No config/preset/reboot writes.
