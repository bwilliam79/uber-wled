...
- **All unit tests are network-free.** Every device response in test code below is a **verbatim capture** from the real controller at `203.0.113.86` (WLED 16.0.0 "Niji", `info.vid = 2605030`, RGBW, 48 LEDs), taken read-only on 2026-07-04. Do not "improve" fixture strings — they encode real edge cases.
...
- [ ] Run `cd $HOME/github/uber-wled/server && npm test -- test/wled/client.test.ts` — expect **pass** (characterization; see note above).
...
- [ ] Run `cd $HOME/github/uber-wled/server && npm test -- test/wled/client.test.ts` — expect all tests pass.
- [ ] Run `cd $HOME/github/uber-wled/server && npm test && npm run build` — expect full suite green and tsc clean (proves `control/routes.ts` v1 callers still compile).
- [ ] Commit: `cd $HOME/github/uber-wled && git add server/src/wled/types.ts server/src/wled/client.ts server/test/wled/client.test.ts && git commit -m "Widen WLED wire types and setState patch to the full state/segment field set"`
...
// Verbatim /json/eff + /json/fxdata entries captured from 203.0.113.86
...
- [ ] Run `cd $HOME/github/uber-wled/server && npm test -- test/wled/capabilities.test.ts` — expect **failure**: `Failed to load .../src/wled/capabilities.js` (module does not exist yet).
- [ ] Create `server/src/wled/capabilities.ts`:
...
- [ ] Run `cd $HOME/github/uber-wled/server && npm test -- test/wled/capabilities.test.ts` — expect all tests pass.
- [ ] Run `cd $HOME/github/uber-wled/server && npm test` — expect full suite green.
- [ ] Commit: `cd $HOME/github/uber-wled && git add server/src/wled/capabilities.ts server/test/wled/capabilities.test.ts && git commit -m "Add fxdata parser producing FxMeta per capability contract"`
...
// 203.0.113.86 (WLED 16.0.0). Names for reference:
...
- [ ] Run `cd $HOME/github/uber-wled/server && npm test -- test/wled/capabilities.test.ts` — expect **failure**: `parsePalettePreviewPage` is not exported (`SyntaxError: The requested module ... does not provide an export named 'parsePalettePreviewPage'`).
...
- [ ] Run `cd $HOME/github/uber-wled/server && npm test -- test/wled/capabilities.test.ts` — expect all tests pass.
- [ ] Run `cd $HOME/github/uber-wled/server && npm test` — expect full suite green.
- [ ] Commit: `cd $HOME/github/uber-wled && git add server/src/wled/capabilities.ts server/test/wled/capabilities.test.ts && git commit -m "Add palx palette-preview parser (stops/random/slots classification)"`
...
describe('wled client v2 reads', () => {
  ...
  it('getPalettePreviews paginates page 0..m inclusive, merging and classifying every page', async () => {
    ...
    // page may be empty (203.0.113.86 serves m=9 with page 9 = {}).
  ...
  it('getConfig returns cfg.json untouched', async () => {
    // Trimmed verbatim from GET /json/cfg on 203.0.113.86.
    const cfg = {
      rev: [1, 0],
      vid: 2605030,
...
- [ ] Run `cd $HOME/github/uber-wled/server && npm test -- test/wled/clientV2.test.ts` — expect **failure**: `does not provide an export named 'getFxData'`.
- [ ] In `server/src/wled/client.ts`: change line 1 to
...
- [ ] Run `cd $HOME/github/uber-wled/server && npm test -- test/wled/clientV2.test.ts` — expect all tests pass.
- [ ] Run `cd $HOME/github/uber-wled/server && npm test && npm run build` — expect green.
- [ ] Commit: `cd $HOME/github/uber-wled && git add server/src/wled/client.ts server/test/wled/clientV2.test.ts && git commit -m "Add WLED client v2 reads: fxdata, paginated palette previews, config, full state"`
...
- [ ] Run `cd $HOME/github/uber-wled/server && npm test -- test/wled/clientV2.test.ts` — expect **failure**: `does not provide an export named 'patchConfig'`.
...
- [ ] Run `cd $HOME/github/uber-wled/server && npm test -- test/wled/clientV2.test.ts` — expect all tests pass.
- [ ] Run `cd $HOME/github/uber-wled/server && npm test && npm run build` — expect green.
- [ ] Commit: `cd $HOME/github/uber-wled && git add server/src/wled/client.ts server/test/wled/clientV2.test.ts && git commit -m "Add WLED client v2 device ops: patchConfig, savePreset, deletePreset, reboot, setNightlight"`
...
- [ ] Run `cd $HOME/github/uber-wled/server && npm test -- test/controllers/capabilitiesRepository.test.ts` — expect **failure**: cannot find module `capabilitiesRepository.js`.
...
- [ ] Run `cd $HOME/github/uber-wled/server && npm test -- test/controllers/capabilitiesRepository.test.ts` — expect all tests pass.
- [ ] Run `cd $HOME/github/uber-wled/server && npm test` — expect full suite green (schema change must not break existing db tests).
- [ ] Commit: `cd $HOME/github/uber-wled && git add server/src/db/schema.ts server/src/controllers/capabilitiesRepository.ts server/test/controllers/capabilitiesRepository.test.ts && git commit -m "Add controller_capabilities table and repository"`
...
- [ ] Run `cd $HOME/github/uber-wled/server && npm test -- test/controllers/capabilityService.test.ts` — expect **failure**: cannot find module `capabilityService.js`.
- [ ] Create `server/src/controllers/capabilityService.ts`:
...
- [ ] Run `cd $HOME/github/uber-wled/server && npm test -- test/controllers/capabilityService.test.ts` — expect all tests pass.
- [ ] Run `cd $HOME/github/uber-wled/server && npm test` — expect full suite green.
- [ ] Commit: `cd $HOME/github/uber-wled && git add server/src/controllers/capabilityService.ts server/test/controllers/capabilityService.test.ts && git commit -m "Add capability refresh service (fetch five datasets, upsert cache)"`
...
- [ ] Run `cd $HOME/github/uber-wled/server && npm test -- test/controllers/statusPoller.test.ts` — expect all 9 tests pass.
- [ ] Run `cd $HOME/github/uber-wled/server && npm test && npm run build` — expect green.
- [ ] Commit: `cd $HOME/github/uber-wled && git add server/src/controllers/statusPoller.ts server/test/controllers/statusPoller.test.ts && git commit -m "Trigger capability refresh from status poller on vid change"`
...
// Verbatim-shaped device responses (values captured from 203.0.113.86,
...
- [ ] Run `cd $HOME/github/uber-wled/server && npm test -- test/controllers/capabilitiesRoute.test.ts` — expect all 4 tests pass.
- [ ] Run `cd $HOME/github/uber-wled/server && npm test && npm run build` — expect full suite green and tsc clean.
- [ ] Commit: `cd $HOME/github/uber-wled && git add server/src/controllers/routes.ts server/test/controllers/capabilitiesRoute.test.ts && git commit -m "Add GET /api/controllers/:id/capabilities with on-demand refresh and 503 fallback"`

---

## Phase A verification gate

- [ ] `cd $HOME/github/uber-wled/server && npm test` — full suite green (all pre-existing tests plus the 50 new ones: 1+14+5+5+7+3+6+5+4 across Tasks 1–9).
- [ ] `cd $HOME/github/uber-wled/server && npm run build` — tsc clean.
- [ ] `cd $HOME/github/uber-wled/client && npm test && npm run build` — untouched by this phase, but must still be green per the master verification gates.
- [ ] Optional live smoke check (read-only GETs only, allowed by the hardware policy): with the dev server running against the real DB, `curl http://localhost:3001/api/controllers/<id>/capabilities` for the controller at `203.0.113.86` and confirm `vid: 2605030`, 220 effects, 72 palettes, and `fxMeta[0].name === 'Solid'`. Do **not** exercise any write function against real hardware.
