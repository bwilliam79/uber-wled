...
**Repo root:** `$HOME/github/uber-wled`. All file paths below are relative to it unless absolute.
...
- [ ] Verify Phase B's actual exports: `grep -n "export" $HOME/github/uber-wled/server/src/control/applyV2.ts` (confirm the entry point is `applyControlPatch` — if the name drifted, use the real one at every call site below) and `grep -n "ps" $HOME/github/uber-wled/server/src/control/applyV2.ts`. Note whether `ps?: number` exists on `ControlPatch` AND is copied into the device body in `writeTarget` (if both, skip the `applyV2.ts` edit in the implementation step below — the test still must pass).
...
- [ ] Run it and confirm the failure is "Cannot find module .../control/actionMap.js": `cd $HOME/github/uber-wled/server && npm test -- test/control/actionMap.test.ts`
...
- [ ] Run the file's tests, expect all green: `cd $HOME/github/uber-wled/server && npm test -- test/control/actionMap.test.ts`
- [ ] Run the whole server suite (guards Phase B regressions from the `ps` addition): `cd $HOME/github/uber-wled/server && npm test`
- [ ] Commit: `cd $HOME/github/uber-wled && git add server/src/control/actionMap.ts server/src/control/applyV2.ts server/test/control/actionMap.test.ts && git commit -m "Add v1-action → fan-out-v2 mapper (actionMap) and ControlPatch.ps"`
...
- [ ] Verify current wiring: `grep -n "applyToMembers\|SchedulerEngine" $HOME/github/uber-wled/server/src/server.ts`
...
- [ ] Run it, expect green already IF Task 1 landed correctly (this test consumes only Task 1 code — if it fails, fix actionMap, not the test): `cd $HOME/github/uber-wled/server && npm test -- test/schedules/engineV2.test.ts`
...
- [ ] Confirm nothing else in `server/src` still imports `applyToMembers`: `grep -rn "applyToMembers" $HOME/github/uber-wled/server/src` → the only remaining hits must be inside `server/src/control/routes.ts` itself (deleted in Task 3).
- [ ] Run the untouched engine suite + the new one + a typecheck-by-build: `cd $HOME/github/uber-wled/server && npm test -- test/schedules/engine.test.ts test/schedules/engineV2.test.ts && npm run build`
- [ ] Commit: `cd $HOME/github/uber-wled && git add server/src/server.ts server/test/schedules/engineV2.test.ts && git commit -m "Migrate scheduler + calendar trigger path onto fan-out v2"`
...
- [ ] Verify what Phase B left in the route: `grep -n "router.post\|members\|targets\|applyToMembers\|ControlAction" $HOME/github/uber-wled/server/src/control/routes.ts`
...
- [ ] Run it, expect failure (v1 branch still answers 200, or an unvalidated 500): `cd $HOME/github/uber-wled/server && npm test -- test/control/routes.test.ts`
...
- [ ] Confirm zero remaining references repo-wide (server side): `grep -rn "applyToMembers\|from './control/routes.js'" $HOME/github/uber-wled/server/src $HOME/github/uber-wled/server/test` → `control/routes.js` may only be imported for `createControlRouter`.
- [ ] Run the full server suite + build: `cd $HOME/github/uber-wled/server && npm test && npm run build` — expect all green.
- [ ] Commit: `cd $HOME/github/uber-wled && git add server/src/control/routes.ts server/test/control/routes.test.ts && git commit -m "Delete v1 {members,action} control route and applyToMembers"`
...
- [ ] Inventory current state (paths will have moved since plan-authoring):
  `cd $HOME/github/uber-wled/client && grep -rn "GroupManager\|ControlPanel" src ; grep -rn "'/api/control/apply'" src ; grep -rn "applyControlV1\b\|ControlAction" src`
...
```
  ...
  Run just those tests and confirm the new assertions fail before the source edit and pass after it: `cd $HOME/github/uber-wled/client && npm test -- src/test` (narrow to the schedule test file the grep found).
- [ ] Delete the two components and their tests:
  `cd $HOME/github/uber-wled && git rm client/src/components/GroupManager.tsx client/src/components/ControlPanel.tsx && git rm --ignore-unmatch client/src/test/GroupManager.test.tsx client/src/test/components/ControlPanel.test.tsx`
...
```bash
cd $HOME/github/uber-wled/client
...
- [ ] Full client gate: `cd $HOME/github/uber-wled/client && npm test && npm run build` — expect green; then re-run the zero-reference check from **Produces** above.
- [ ] Commit: `cd $HOME/github/uber-wled && git add -A client/src && git commit -m "Delete v1 client control path: GroupManager, ControlPanel, ControlAction, orphans"`
...
- [ ] Run: `cd $HOME/github/uber-wled/client && npm test` — expect green (import paths are the only change; no behavior change).
- [ ] Verify zero references to the old path: `grep -rn "components/FirmwareStatus\|components/AssetPickerModal" $HOME/github/uber-wled/client/src` → expect no hits.
- [ ] Run: `cd $HOME/github/uber-wled/client && npm run build` — expect green.
- [ ] Commit: `cd $HOME/github/uber-wled && git add -A client/src && git commit -m "Phase I: relocate FirmwareStatus/AssetPickerModal into sections/devices/"`
...
- [ ] Sanity-check internal accuracy against the shipped app: `grep -rn "live poll" $HOME/github/uber-wled/client/src/sections/settings` (Settings row exists), `grep -n "network_mode" $HOME/github/uber-wled/docker-compose.yml`, and confirm the section names in the README match the shell's nav labels: `grep -rn "label:" $HOME/github/uber-wled/client/src/components/AppShell.tsx $HOME/github/uber-wled/client/src/components/*.tsx | grep -i "home\|layout\|devices\|themes\|schedule\|firmware\|settings"`. Fix any label mismatch in the README (the app is the source of truth).
- [ ] Commit: `cd $HOME/github/uber-wled && git add README.md && git commit -m "Rewrite README for the 1.0 seven-section control plane"`
...
- [ ] Verify the define still exists post-Phase-C: `grep -n "__APP_VERSION__" $HOME/github/uber-wled/client/vite.config.ts $HOME/github/uber-wled/client/src -r` — expect the `define` in vite.config.ts and at least one render site. If Phase C dropped the render site, add `<span className="sidebar-version">v{__APP_VERSION__}</span>` back into the shell's sidebar brand block (and `.sidebar-version { font-size: 11px; color: var(--text-muted); }` if the class is gone).
- [ ] Edit `client/package.json`: set `"version": "1.0.0"`.
- [ ] Edit `server/package.json`: set `"version": "1.0.0"`.
- [ ] Sync both lockfiles: `cd $HOME/github/uber-wled/client && npm install --package-lock-only && cd $HOME/github/uber-wled/server && npm install --package-lock-only`
- [ ] Prove the bundle picked it up: `cd $HOME/github/uber-wled/client && npm run build && grep -o '"1.0.0"' dist/assets/*.js | head -1` — expect `"1.0.0"`.
- [ ] Run both suites (version bumps must not break anything): `cd $HOME/github/uber-wled/server && npm test && cd $HOME/github/uber-wled/client && npm test`
- [ ] Commit: `cd $HOME/github/uber-wled && git add client/package.json client/package-lock.json server/package.json server/package-lock.json && git commit -m "Version 1.0.0"`
...
//     deviceHost  WLED controller host    (default 203.0.113.86)
...
async function main() {
  const deviceHost = process.argv[2] ?? '203.0.113.86';
...
- [ ] Hardware-free verification (syntax + the pure restore builder, using a real state fixture captured read-only from 203.0.113.86 — `on:true, bri:9, transition:7`, seg0 white RGBW):

```bash
cd $HOME/github/uber-wled && node --check scripts/hw-smoke.mjs && node --input-type=module -e "
...
- [ ] Commit: `cd $HOME/github/uber-wled && git add scripts/hw-smoke.mjs && git commit -m "Add reversible hardware smoke script (capture → apply v2 → restore)"`
...
- [ ] `cd $HOME/github/uber-wled/server && npm test` — expect exit 0. Record the vitest summary line (`Test Files N passed`, `Tests M passed`).
- [ ] `cd $HOME/github/uber-wled/client && npm test` — expect exit 0. Record the summary line.
- [ ] `cd $HOME/github/uber-wled/client && npm run build` — expect exit 0.
- [ ] `cd $HOME/github/uber-wled && docker build -t uber-wled:1.0.0 .` — expect a successful image build (this validates both `npm run build`s inside the multi-stage Dockerfile).
...
- [ ] Commit: `cd $HOME/github/uber-wled && git add README.md && git commit -m "True up README test counts from measured 1.0.0 suite runs"`
...
- [ ] Start the stack: terminal 1 `cd $HOME/github/uber-wled/server && npm run dev`; terminal 2 `cd $HOME/github/uber-wled/client && npm run dev`. Open the Vite URL in the browser.
...
- [ ] With the dev server from Task 9 still running: `cd $HOME/github/uber-wled && node scripts/hw-smoke.mjs 203.0.113.86 http://localhost:3000`
...
- [ ] If it exits 1 with `RESTORE MISMATCH`, the device is NOT clean: re-run `node scripts/hw-smoke.mjs 203.0.113.86 http://localhost:3000` (restore runs unconditionally) and debug the apply path before any deploy.
...
- [ ] Push everything: `cd $HOME/github/uber-wled && git push origin main`
