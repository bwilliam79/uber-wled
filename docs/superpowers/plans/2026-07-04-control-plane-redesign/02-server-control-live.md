...
- Single file: `cd $HOME/github/uber-wled/server && npm test -- <file>` (the `test` script is `vitest run`, so the extra arg is a vitest filename filter).
- Full suite (run before every commit): `cd $HOME/github/uber-wled/server && npm test`

## Real-device fixture provenance

All fixtures in this plan were probed read-only from the real controller at `http://203.0.113.86` on 2026-07-04 (WLED 16.0.0 "Niji", `vid` 2605030, ESP32, 48 LEDs in two segments 0–39 and 39–48, RGBW). Never POST to that device while implementing this plan.
...
- [ ] Run: `cd $HOME/github/uber-wled/server && npm test -- test/db/schema.test.ts` — expect FAIL: `expected [...] to include 'icon'` (and, if Phase A did not ship yet, `no such table: controller_capabilities` from `PRAGMA` returning empty → arrayContaining failure).
...
- [ ] Run: `cd $HOME/github/uber-wled/server && npm test -- test/db/schema.test.ts` — expect PASS (4 tests).
...
- [ ] Run: `cd $HOME/github/uber-wled/server && npm test -- test/settings/routes.test.ts` — expect FAIL: `expected { … } to deeply equal { … livePollIntervalSeconds: 2 }`.
...
- [ ] Run: `cd $HOME/github/uber-wled/server && npm test -- test/settings/routes.test.ts` — expect PASS. Then full suite: `cd $HOME/github/uber-wled/server && npm test` — expect PASS (no other test asserts the exact settings shape; `test/db/client.test.ts` and repo tests are shape-agnostic).

- [ ] Commit: `cd $HOME/github/uber-wled && git add server && git commit -m "Phase B task 1: schema adds (groups.icon/sort_order, settings.live_poll_interval_seconds) + livePollIntervalSeconds setting"`
...
- [ ] Run: `cd $HOME/github/uber-wled/server && npm test -- test/groups/routes.test.ts` — expect FAIL: `expected undefined to be 'lamp'` (icon not returned) and `404`-shaped failures for `/reorder`.
...
- [ ] Run: `cd $HOME/github/uber-wled/server && npm test -- test/groups/routes.test.ts` — expect PASS (8 tests). Then the full suite: `cd $HOME/github/uber-wled/server && npm test` — expect PASS (schedules/calendar tests construct groups via `repo.add({name, members})`, which still type-checks since the new fields are optional).

- [ ] Commit: `cd $HOME/github/uber-wled && git add server && git commit -m "Phase B task 2: groups icon + sortOrder + POST /api/groups/reorder"`
...
- [ ] Write the failing test `server/test/control/applyV2.test.ts`:
  ...
  // controller at 203.0.113.86 (WLED 16.0.0 "Niji", vid 2605030).
...
- [ ] Run: `cd $HOME/github/uber-wled/server && npm test -- test/control/applyV2.test.ts` — expect FAIL: `Cannot find module '.../src/control/applyV2.js'`.
...
- [ ] Run: `cd $HOME/github/uber-wled/server && npm test -- test/control/applyV2.test.ts` — expect PASS (13 tests).

- [ ] Commit: `cd $HOME/github/uber-wled && git add server && git commit -m "Phase B task 3: applyV2 contract types, target expansion, per-device name resolution"`
...
- [ ] Append failing write-path tests to `server/test/control/applyV2.test.ts` (add `vi`, `afterEach` to the vitest import; add `applyControlPatch` to the applyV2 import):
  ...
  // Real two-segment layout probed from 203.0.113.86 (segments 0-39 and 39-48).
  const LIVE_STATE = {
    on: true, bri: 9, ps: -1,
    seg: [
...
- [ ] Run: `cd $HOME/github/uber-wled/server && npm test -- test/control/applyV2.test.ts` — expect FAIL: `applyControlPatch` is not exported.
...
- [ ] Run: `cd $HOME/github/uber-wled/server && npm test -- test/control/applyV2.test.ts` — expect PASS (20 tests).
...
- [ ] Run: `cd $HOME/github/uber-wled/server && npm test -- test/control/routes.test.ts` — expect FAIL: v2 body falls into the v1 destructuring and 500s/misbehaves (`Cannot read properties of undefined` or empty results).
...
- [ ] Run: `cd $HOME/github/uber-wled/server && npm test -- test/control/routes.test.ts` — expect PASS (8 tests: 4 v1 + 4 v2). Full suite: `cd $HOME/github/uber-wled/server && npm test` — expect PASS (schedules engine/calendar drive `applyToMembers` directly and are untouched).

- [ ] Commit: `cd $HOME/github/uber-wled && git add server && git commit -m "Phase B task 4: fan-out v2 applyControlPatch + POST /api/control/apply v2/v1 discrimination"`
...
- [ ] Write the failing test `server/test/live/sessions.test.ts`:
  ...
  // Real shapes probed from 203.0.113.86 (WLED 16.0.0).
...
- [ ] Run: `cd $HOME/github/uber-wled/server && npm test -- test/live/sessions.test.ts` — expect FAIL: `Cannot find module '.../src/live/sessions.js'`.
...
- [ ] Run: `cd $HOME/github/uber-wled/server && npm test -- test/live/sessions.test.ts` — expect PASS (7 tests).

- [ ] Commit: `cd $HOME/github/uber-wled && git add server && git commit -m "Phase B task 5: refcounted live fast-poll session manager"`
...
- [ ] Run: `cd $HOME/github/uber-wled/server && npm test -- test/live/routes.test.ts` — expect FAIL: `Cannot find module '.../src/live/routes.js'`.
...
- [ ] Run: `cd $HOME/github/uber-wled/server && npm test -- test/live/routes.test.ts` — expect PASS (4 tests). Full suite: `cd $HOME/github/uber-wled/server && npm test` — expect PASS.

- [ ] Commit: `cd $HOME/github/uber-wled && git add server && git commit -m "Phase B task 6: GET /api/live SSE stream with heartbeat and disconnect cleanup"`
...
- [ ] Write the failing test `server/test/devices/presets.test.ts`:
  ...
  // A fresh WLED 16.0.0 device serves {"0":{}} (probed live at 203.0.113.86).
...
- [ ] Run: `cd $HOME/github/uber-wled/server && npm test -- test/devices/presets.test.ts` — expect FAIL: `Cannot find module '.../src/devices/presets.js'`.
...
- [ ] Run: `cd $HOME/github/uber-wled/server && npm test -- test/devices/presets.test.ts` — expect PASS (3 tests).

- [ ] Commit: `cd $HOME/github/uber-wled && git add server && git commit -m "Phase B task 7: device preset list parser (isPlaylist + quicklook)"`
...
- [ ] Write the failing test `server/test/devices/configDiff.test.ts`:
  ...
  // Subset of the real /json/cfg probed live from 203.0.113.86 (WLED 16.0.0).
  const CURRENT = {
...
- [ ] Run: `cd $HOME/github/uber-wled/server && npm test -- test/devices/configDiff.test.ts` — expect FAIL: `Cannot find module '.../src/devices/configDiff.js'`.
...
- [ ] Run: `cd $HOME/github/uber-wled/server && npm test -- test/devices/configDiff.test.ts` — expect PASS (7 tests).

- [ ] Commit: `cd $HOME/github/uber-wled && git add server && git commit -m "Phase B task 8: flat config diff builder + rebootRequired rule"`
...
- [ ] Run: `cd $HOME/github/uber-wled/server && npm test -- test/devices/routes.test.ts` — expect FAIL: `Cannot find module '.../src/devices/routes.js'`.
...
- [ ] Run: `cd $HOME/github/uber-wled/server && npm test -- test/devices/routes.test.ts` — expect PASS (7 tests). Full suite: `cd $HOME/github/uber-wled/server && npm test` — expect PASS (confirms the new mount steals no existing controller routes).

- [ ] Commit: `cd $HOME/github/uber-wled && git add server && git commit -m "Phase B task 9: device preset routes (list/save-with-slot/delete)"`
...
- [ ] Append failing tests to the outer describe in `server/test/devices/routes.test.ts`:
  ...
  ```ts
    // Subset of the real /json/cfg probed from 203.0.113.86.
...
- [ ] Run: `cd $HOME/github/uber-wled/server && npm test -- test/devices/routes.test.ts` — expect FAIL: `404` (routes not defined yet — the router falls through).
...
- [ ] Run: `cd $HOME/github/uber-wled/server && npm test -- test/devices/routes.test.ts` — expect PASS (13 tests). Full suite: `cd $HOME/github/uber-wled/server && npm test` — expect PASS.

- [ ] Commit: `cd $HOME/github/uber-wled && git add server && git commit -m "Phase B task 10: config get/dry-run-diff/apply + reboot routes"`
...
- [ ] Run: `cd $HOME/github/uber-wled/server && npm test -- test/segments/routes.test.ts` — expect FAIL: existing PUT test fails on the body mismatch (no `udpn`), new routes 404.
...
- [ ] Run: `cd $HOME/github/uber-wled/server && npm test -- test/segments/routes.test.ts` — expect PASS (8 tests).

- [ ] Phase gate: `cd $HOME/github/uber-wled/server && npm test` — expect ALL PASS — and `cd $HOME/github/uber-wled/server && npm run build` — expect a clean tsc build.

- [ ] Commit: `cd $HOME/github/uber-wled && git add server && git commit -m "Phase B task 11: segments routes widened to full field set + create/delete with udpn nn"`
